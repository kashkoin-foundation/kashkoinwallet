...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("kashkoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "kashkoin:");
    }
    if(!_request.startsWith("kashkoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with kashkoin:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "jkhkdpe4fbJGzxJ48YJJG7C5wMg8q8QzgL2gRB6ScBRASQr3SJDaRXBHjGBVRkiLHohK97Jz6U5n6C8pg7uunDKt5wRRvULHEX";
